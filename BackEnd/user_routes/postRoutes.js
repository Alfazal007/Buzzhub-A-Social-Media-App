...
router.get("/feeds/all", async (req, res) => {
    try {
        ...
        try {
            ...
            if (user) {
                const myPosts = await Post.find({ userId: user._id }).select("userId description img").sort({ createdAt: -1 }).lean()
                myPosts.forEach(post => post.username = username1)
                const followingPosts = await Promise.all(
                    user.following.map((friendId) => {
                        return Post.find({ userId: friendId }).select("userId description img").sort({ createdAt: -1 }).lean().then(posts => {
                            posts.forEach(post => post.username = usernameMap[post.userId])
                            return posts
                        })
                    })
                )
                ...
                return res.status(403).json('Login to see feed')
            }
        } catch (err) {
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        try {
            ...
            const user = await User.findById(id)
            if (user) {
                const postToDelete = await Post.findById(req.params.id).select("userId description")
                if (postToDelete) {
        ...
        try {
            ...
            if (user) {
                ...
                if (user._id.equals(post.userId)) {
                    ...
                    const updatedPost = await Post.findOneAndUpdate({ _id: req.params.id }, changeObject, { new: true, runValidators: true })
                    res.status(200).json(updatedPost)
                } else {
                    return res.status(404).json("Login again with valid credentials to edit post")
                }
            } else {
                return res.status(404).json("Login again with valid credentials to edit post")
            }
            ...
            if (user) {
                ...
                if (post) {
                    if (!post.likes.includes(userIdLikingThePost)) {
                        const updatedPost = await Post.findOneAndUpdate({ _id: req.params.id }, { $push: { likes: userIdLikingThePost } })
            ...
            if (user) {
                ...
                if (post) {
                    if (post.likes.includes(userIdDislikingThePost)) {
                        const updatedPost = await Post.findOneAndUpdate({ _id: req.params.id }, { $pull: { likes: userIdDislikingThePost } })
